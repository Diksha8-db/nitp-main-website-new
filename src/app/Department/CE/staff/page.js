...
const CivilStaffpage = () => {
  const [staffList, setStaffList] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(false);

  useEffect(() => {
    const fetchCivilStaff = async () => {
      try {
        setLoading(true);
        setError(false);
        ...
        do {
          const { data } = await axios.get(
            `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/staff2?type=all&department=ce&page=${page}&limit=50`
          );
        ...
        console.log("Fetched Civil staff:", allStaff);
      } catch (err) {
        console.log(err);
        setError(true);
      } finally {
      ...
      <div className="mt-5">
        ...
        {loading ? (
          ...
          <p className="text-center text-red-500 mt-6">
            Sorry, failed to fetch the Civil Engineering staff data.
          </p>
        ) : staffList.length === 0 ? (
