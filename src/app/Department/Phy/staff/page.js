      ...
      try {
        ...
        do {
          const { data } = await axios.get(
            `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/staff2?type=all&department=phy&page=${page}&limit=50`,
          );
        ...
        console.log("Fetched PHY staff:", allStaff);
      } catch (err) {
        console.log(err);
        setError(true);
      } finally {
      ...
      <div className="mt-5">
        ...
        {loading ? (
          ...
          <p className="text-center text-red-500 mt-6">
            Sorry, failed to fetch the Applied Physics and Material Engineering
            staff data.
          </p>
        ) : staffList.length === 0 ? (
